<?php
class Ad08model extends CI_Model
{
    ...
    {   //메인 슬라이드
        $sql = "SELECT yutube_idx, yutube_link
         FROM ad08_yutube ORDER BY yutube_idxs DESC";
    ...
    {   //메인 슬라이드
        $sql = "SELECT slide_idx, slide_image, slide_link, slide_name
         FROM ad08_slide ORDER BY slide_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return' => true, 'list' => $list);
    }
    ...
    {   //이벤트
        $sql = "SELECT event_idx, event_image, event_link, event_name
         FROM ad08_event ORDER BY event_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return' => true, 'list' => $list);
    } 
    ...
    {  //메인 리스트
        ...
        //                product_hashtag FROM ad08_product
    ...
    public function  callData($idx)
    {
        $sql = "SELECT product_idx, product_slide, product_name, insure_call,
                ...
                product_thumb,
                product_get FROM ad08_product JOIN ad08_insure
                USING(product_idx) WHERE product_idx = ?";
        $array = array($idx);
        $data = $this->db->query($sql, $array)->row();
        return array('return' => true, 'data' => $data);
    ...
    {
        $sql = "SELECT faq_idx, faq_question, faq_answer FROM ad08_faq ORDER BY faq_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return' => true, 'list' => $list);
    }
    ...
    {
        $array = array($idx);
        $sql = "SELECT product_get FROM ad08_product WHERE product_idx = ?";
        $count = $this->db->query($sql, $array)->row()->product_get;
        $count += 1;
        $array = array($count, $idx);
        $sql = "UPDATE ad08_product SET product_get = ? WHERE product_idx = ?";
    ...
    public function  newsDataView($idx)
    {
        ...
        $sql = "SELECT * FROM ad08_news WHERE news_idx = (SELECT MIN(news_idx) min FROM ad08_news WHERE news_idx > ? AND news_show = 1)";
        $array = array($idx);
        $min = $this->db->query($sql, $array)->row()->news_idx;

        $sql = "SELECT * FROM ad08_news WHERE news_idx = (SELECT MAX(news_idx) FROM ad08_news WHERE news_idx < ? AND news_show = 1)";
        $array = array($idx);
