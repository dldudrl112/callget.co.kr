<?php
class Ad04model extends CI_Model {
    ...
    public function yutubeData() {   //메인 슬라이드
        $sql="SELECT yutube_idx, yutube_link
         FROM ad04_yutube ORDER BY yutube_idxs DESC";
    ...
    public function SlideData() {   //메인 슬라이드
        $sql="SELECT slide_idx, slide_image, slide_link, slide_name
         FROM ad04_slide ORDER BY slide_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return'=>true,'list'=>$list);
    }
    ...
    public function category2Data() {   //메인 카테고리2
...
//                product_hashtag FROM ad04_product
...
//                limit {$limit} offset {$offset} ";
    ...
    public function mainList2($page = 1, $idx2 = null) {  //메인 리스트
        $limit=6;
        $offset=$limit*($page-1);
        $where = '';
        if($idx2) {
            $where = " AND category_idx2 = ".$idx2;
        }
//        $sql="SELECT product_idx, TO_DAYS(CAST(product_time AS DATE)) -
//                TO_DAYS(CAST(now() AS DATE)) AS dates,
//                product_get, product_logo, insure_logo, product_thumb,
...
//                limit {$limit} offset {$offset} ";
    ...
    public function  callData($idx) {
        $sql="SELECT product_idx, product_slide, product_name, insure_call,
                ...
                product_thumb,
                product_get FROM ad04_product JOIN ad04_insure
                USING(product_idx) WHERE product_idx = ?";
        $array= array($idx);
        $data = $this->db->query($sql, $array)->row();
        return array('return'=>true,'data'=>$data);
    ...
    public function  faqData() {
        $sql="SELECT faq_idx, faq_question, faq_answer FROM ad04_faq ORDER BY faq_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return'=>true,'list'=>$list);
    }
    public function  updateGet($idx) {
        $array = array($idx);
        $sql="SELECT product_get FROM ad04_product WHERE product_idx = ?";
        $count = $this->db->query($sql, $array)->row()->product_get;
        $count += 1;
        $array = array($count, $idx);
        $sql="UPDATE ad04_product SET product_get = ? WHERE product_idx = ?";
...
////소식 페이지
    ...
    public function  newsDataView($idx) {
        ...
        $sql = "SELECT * FROM ad04_news WHERE news_idx = (SELECT MIN(news_idx) min FROM ad04_news WHERE news_idx > ? AND news_show = 1)";
        $array = array($idx);
        $min = $this->db->query($sql, $array)->row()->news_idx;

        $sql = "SELECT * FROM ad04_news WHERE news_idx = (SELECT MAX(news_idx) FROM ad04_news WHERE news_idx < ? AND news_show = 1)";
        $array = array($idx);
